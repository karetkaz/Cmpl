...
$CMPL_BIN/cmpl -dump.scite extras/cmplGfx.api "$BIN/libGfx.so" lib/gfxlib.ci
...
$CMPL_BIN/cmpl -X-stdin-steps -profile/t/P/G/M -api/A/m/d/p -asm/g/n/s -ast/t -doc -use -dump.json "extras/test.prof.json" -stdlib/stdlib.ci "test/test.ci"
...
$CMPL_BIN/cmpl -X-stdin+steps -asm/n/s -run/g -log/d "$DUMP_FILE" "$CMPL_BIN/libFile.so" "$CMPL_BIN/libGfx.so" "$CMPL_HOME/lib/gfxlib.ci" "$CMPL_HOME/test/test.ci"
for file in $(echo "$TEST_FILES")
do
	cd $(dirname "$file")
	echo "**** running test: $file"
	$CMPL_BIN/cmpl -X-stdin+steps -run/g -log/a/d "$DUMP_FILE" "$CMPL_BIN/libFile.so" "$CMPL_BIN/libOpenGL.so" "$CMPL_BIN/libGfx.so" "$CMPL_HOME/lib/gfxlib.ci" "$file"
	if [ "$?" -ne "0" ]; then
		echo "******** failed: $file"
	fi
done
