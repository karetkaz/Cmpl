...
$CMPL_BIN/cmpl -X+steps+fold+fast+asgn-stdin-glob-offsets -debug/G/M -api/a/m/d/p -asm/n/s -ast -doc -log/d/15 "extras/test.dump.ci" -dump.ast.xml "extras/test.dump.xml" "test/test.ci"

# dump symbols, documentation, assembly, syntax tree and global variables (to be compared with previous version to test if the code is generated properly)
$CMPL_BIN/cmpl -X+steps+fold+fast+asgn-stdin-glob-offsets -debug/G/M -api/a/m/d/p/u -asm/n/s -ast -doc -log/d "extras/libs.dump.ci" "$BIN/libFile.so" "$BIN/libGfx.so" "cmplGfx/gfxlib.ci"
...
TEST_FILES="$TEST_FILES $CMPL/cmplFile/test/*.ci"
TEST_FILES="$TEST_FILES $CMPL/cmplGfx/test/*.ci"
TEST_FILES="$TEST_FILES $CMPL/cmplGL/test/*.ci"

DUMP_FILE=$CMPL_BIN.dump.ci
$CMPL_BIN/cmpl -X-stdin+steps -asm/n/s -run/g -log/d "$DUMP_FILE" -std"$CMPL/lib/stdlib.ci" "$CMPL_BIN/libFile.so" "$CMPL_BIN/libGfx.so" "$CMPL/cmplGfx/gfxlib.ci" "$CMPL/test/test.ci"
for file in $(echo "$TEST_FILES")
do
	cd $(dirname "$file")
	echo "**** running test: $file"
	$CMPL_BIN/cmpl -X-stdin+steps -run/g -log/a/d "$DUMP_FILE" -std"$CMPL/lib/stdlib.ci" "$CMPL_BIN/libFile.so" "$CMPL_BIN/libOpenGL.so" "$CMPL_BIN/libGfx.so" "$CMPL/cmplGfx/gfxlib.ci" "$file"
	if [ "$?" -ne "0" ]; then
		echo "******** failed: $file"
	fi
done
