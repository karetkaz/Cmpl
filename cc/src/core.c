	...
	switch (ast->kind) {
		...
		case STMT_if:  {
			...
			if (tt && (rt->vm.opti || ast->cst2 == QUAL_sta)) {	// static if then else
				...
				// local declarations inside a static if should grow the stack.
				stpos = stkoffs(rt, 0);
			}
...
static void install_type(ccState cc, int mode) {
	...
	install(cc, "int32ptr", ATTR_const | TYPE_def, TYPE_ref, 4, type_i32, NULL);

	if (1) {	// if pointer is needed.
		type_ptr = install(cc, "pointer", ATTR_const | TYPE_rec, TYPE_ref, 4, type_rec, NULL);
		null_ref = install(cc,    "null", ATTR_const | TYPE_ref, TYPE_any, 4, type_ptr, NULL);
	}
	...
	if (1) {	// if aliases are needed.
		...
		if ((type_str = install(cc, "string", ATTR_const | TYPE_arr, TYPE_ref, 4, type_chr, NULL))) {
			/* TODO:
...
static void install_emit(ccState cc, int mode) {
	...
	if (emit_opc && (mode & creg_eopc) == creg_eopc) {
		...
		ref = install(cc, "byRef", TYPE_rec, TYPE_ref, 4, NULL, NULL);
...
ccState ccInit(state rt, int mode, int libcHalt(state)) {
	...
	if (cc->type_rec != NULL) {
		symn arg = NULL;
		enter(cc, NULL);
		if ((arg = install(cc, "size", ATTR_const | TYPE_ref, TYPE_any, 4, type_i32, NULL))) {
...
static void install_lang(ccState cc, unsigned level) {
	...
	if ((cls = ccBegin(rt, "Runtime"))) {
		...
		install(cc, "debug(string message, bool trace, bool error, string args...)", TYPE_def, 0, 0);
		install(cc, "debug(string message, string args...) = debug(message, false, false, args)", TYPE_def, 0, 0);
		install(cc, "trace(string message, string args...) = debug(message, true, false, args)", TYPE_def, 0, 0);
		install(cc, "error(string message, string args...) = debug(message, false, true, args)", TYPE_def, 0, 0);
		install(cc, "info(string message, string args...) = debug(message, false, false, args)", TYPE_def, 0, 0);
		install(cc, "log(string message, string args...) = debug(message, false, false, args)", TYPE_def, 0, 0);
		install(cc, "assert(bool test, string message, string args...) = if (test) debug(message, true, true, args)", TYPE_def, 0, 0);
		ccEnd(cc, cls);
	}
