	...
	if (chr != -1) switch (chr) {
		...
		read_idf: {			// [a-zA-Z_][a-zA-Z0-9_]*
			...
			*ptr = 0;
			...
			for (key = 0; key < tok_last; key += 1) {
				const char *str = tok_tbl[key].name;

				if (tok_tbl[key].type != 0xff)
					continue;
...
#define debugEnterFun //debug("(%d)%I`%k`", line(s), s->nest, peek(s))
//{~~~~~~~~~ Parser ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
