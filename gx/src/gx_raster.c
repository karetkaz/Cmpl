...
static void colcpy_32mix(void* dst, void *src, void *lut, size_t cnt) {
	...
	if (alpha == (size_t)-1) {
		for ( ;cnt > 0; cnt -= 1, dst += 4, src += 4) {
			register uint32_t val = *(uint32_t*)src;
			val = gx_mixcolor(*(int32_t*)dst, val, val >> 24);
			*(int32_t*)dst = val;
		}
	}
	else {
		for ( ;cnt > 0; cnt -= 1, dst += 4, src += 4) {
			register uint32_t val = *(uint32_t*)src;
			val = gx_mixcolor(*(int32_t*)dst, val, (uint8_t)alpha);
			*(int32_t*)dst = val;
		}
	}
}
...
void colset_32mix(void* dst, void *src, void *lut, size_t cnt) {
	size_t alpha = (size_t)lut;
	for ( ;cnt > 0; cnt -= 1, dst += 4, src += 1) {
		*(int32_t*)dst = gx_mixcolor(*(int32_t*)dst, (int32_t)alpha, *(uint8_t*)src);
	}
}

static void colcpy_24_32(void* dst, void *src, void *lut, size_t cnt) {
...
//				case cblt_conv_16: return colcpy_16_16;
...
//				case cblt_conv_15: return colcpy_15_15;
...
//				case cblt_conv_08: return colcpy_08_08;
			}
			break;
