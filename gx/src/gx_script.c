...
static gxSurfHnd newSurf(int w, int h) {
	...
	while (hnd < surfCount) {
		if (!surfaces[hnd].depth) {
			if (gx_initSurf(&surfaces[hnd], w, h, 32, Surf_recycle) == 0) {
				return hnd + 1;
			}
			break;
		}
	...
	switch ((surfFunc)rt->data) {
		...
		case surfOpDrawRect:
			...
			if (surf != NULL) {
				...
				int y1 = popi32(rt);
				int col = popi32(rt);
				g2_drawrect(surf, x0, y0, x1, y1, col);
				return noError;
			}
			break;
		case surfOpFillRect:
			...
			if (surf != NULL) {
				...
				int y1 = popi32(rt);
				int col = popi32(rt);
				g2_fillrect(surf, x0, y0, x1, y1, col);
				return noError;
			}
			break;
		case surfOpDrawOval:
		...
		case surfOpDrawText:
			...
			if (surf != NULL) {
				...
				char *str = popstr(rt);
				int col = popi32(rt);
				g2_drawText(surf, x0, y0, &font, str, col);
				return noError;
			}
			break;
		...
		case surfOpGradSurf: {
			...
			if (sdst) {
				gx_gradsurf(sdst, roi, lut, mode, repeat);
				reti32(rt, dst);
				return noError;
			}
		} break;
