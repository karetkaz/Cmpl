...
/**
 * Initialize virtual machine context.
 * 
 * @param ctx Runtime context.
 ...
 */
vmError execute(rtContext ctx, int argc, char *argv[], void *extra);
 ...
 */
vmError invoke(rtContext ctx, symn fun, void *ret, void *args, const void *extra);
 ...
 * @usage see also: test.gl/gl.c
	...
	if (!rt->api.ccDefCall(rt, setMouseCb, NULL, "void setMouseCallback(void Callback(int32 btn, int32 x, int32 y)")) {
		error...
	}
 */
symn rtLookupSym(rtContext ctx, size_t offs, int callsOnly);
 ...
 * @note Invocation to `execute` must precede this call.
 */
void *rtAlloc(rtContext ctx, void *ptr, size_t size, void dbg(dbgContext rt, void *mem, size_t size, char *kind));
 ...
 */
size_t vmOffset(rtContext ctx, void *ptr);
 ...
 * @return
 */
int fixJump(rtContext ctx, size_t src, size_t dst, ssize_t stc);
...
/**
 * Emit an instruction with int argument.
 * 
 * @param ctx Runtime context.
 * @param opc Opcode.
