...
void *setBuff(struct arrBuffer *buff, size_t idx, const void *data) {
	...
	if (pos >= buff->cap) {
		size_t size = pos * 2;
		buff->ptr = realloc(buff->ptr, size);
		if (buff->ptr == NULL) {
			return NULL;
		}
		buff->cap = size;
...
void *insBuff(struct arrBuffer *buff, size_t idx, void *data) {
	if (idx >= buff->cnt) {
		// insert at or after the end
		return setBuff(buff, idx, data);
	}

	// insertion into the middle
	if (buff->cnt * buff->esz >= buff->cap) {
		size_t cap = buff->cap * 2;
		void *ptr = realloc(buff->ptr, cap);
		if (ptr == NULL) {
			return NULL;
		}
		buff->ptr = ptr;
	...
	void *ptr = buff->ptr + idx * buff->esz;
	memmove(ptr + buff->esz, ptr, buff->esz * (buff->cnt - idx));
	if (data != NULL) {
...
unsigned rehash(const char *data, size_t size) {
	...
	unsigned hs = 0xffffffff;
	while (size-- > 0) {
		hs = (hs >> 8) ^ crc_tab[(uint8_t) (hs ^ (*data++))];
	}
