...
%BIN%\cmpl -X-stdin+steps -asm/n/s -run/g -log/d "%DUMP_FILE%" "%BIN%\libFile.dll" "%BIN%\libOpenGL.dll" "%BIN%\libGfx.dll" "%CMPL_HOME%\cmplGfx\gfxlib.ci" "%CMPL_HOME%\cmplStd\test\test.ci"
FOR %%f IN (%TEST_FILES%) DO (
	pushd "%%~dpf"
	echo **** running test: %%f
	%BIN%\cmpl -X-stdin+steps -run/g -log/a/d "%DUMP_FILE%" "%BIN%\libFile.dll" "%BIN%\libOpenGL.dll" "%BIN%\libGfx.dll" "%CMPL_HOME%\cmplGfx\gfxlib.ci" "%%f"
