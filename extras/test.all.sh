...
$CMPL_BIN/cmpl -dump.scite extras/cmpl.api "$BIN/libFile.so" "$BIN/libOpenGL.so" "$BIN/libGfx.so" cmplGfx/gfxlib.ci
if [ "$?" -ne "0" ]; then
	exit 1
fi
...
# dump symbols, documentation, assembly, syntax tree and global variables (to be compared with previous version to test if the code is generated properly)
$CMPL_BIN/cmpl -X+steps+fold+fast-stdin-glob-offsets -debug/G/M -api/A/m/d/p -asm/n/s -ast -doc -use -log/d "extras/dump/libs.dump.ci" -stdcmplStd/stdlib.ci "$BIN/libFile.so" "$BIN/libGfx.so" "cmplGfx/gfxlib.ci"
...
$CMPL_BIN/cmpl -X-stdin+steps -asm/n/s -run/g -log/d "$DUMP_FILE" "$CMPL_BIN/libFile.so" "$CMPL_BIN/libOpenGL.so" "$CMPL_BIN/libGfx.so" "$CMPL_HOME/cmplGfx/gfxlib.ci" "$CMPL_HOME/cmplStd/test/test.ci"
for file in $(echo "$TEST_FILES")
do
	cd $(dirname "$file")
	echo "**** running test: $file"
	$CMPL_BIN/cmpl -X-stdin+steps -run/g -log/a/d "$DUMP_FILE" "$CMPL_BIN/libFile.so" "$CMPL_BIN/libOpenGL.so" "$CMPL_BIN/libGfx.so" "$CMPL_HOME/cmplGfx/gfxlib.ci" "$file"
	if [ "$?" -ne "0" ]; then
		echo "******** failed: $file"
	fi
done
