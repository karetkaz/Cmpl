...
int gx_cMatSurf(gx_Surf surf, gx_Rect roi, const int32_t mat[16]) {
	...
	for (int y = 0; y < rect.h; y += 1) {
		...
		for (int x = 0; x < rect.w; x += 1) {
			...
			const int g = cBuff->g;
			const int b = cBuff->b;
			const int a = 256; // cBuff->a;
			int ro = (r * mat[0x0] + g * mat[0x1] + b * mat[0x2] + a * mat[0x3]) >> 16;
			int go = (r * mat[0x4] + g * mat[0x5] + b * mat[0x6] + a * mat[0x7]) >> 16;
			int bo = (r * mat[0x8] + g * mat[0x9] + b * mat[0xa] + a * mat[0xb]) >> 16;
			//int ba = (r * mat[0xc] + g * mat[0xd] + b * mat[0xe] + a * mat[0xf]) >> 16;
